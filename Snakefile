...
spdir = 'KL20_nereocystis/' #species specific folder in datadir
...
# input data
metadata = datadir + spdir + 'geographicCoords_KL20_nereocystis.txt' 
prefix = 'JARUPZ0100000{CHR}.1_popsize_params01'
...
trees_per_chr = [28118, 16956, 16553, 13043, 12361, 15495, 14149,  6627, 13012,
		 ...
		 3568,  1999,  5012] #calculated in plots.ipynb from second line in anc files
treeskip = 10000 #number of trees to skip along genome (thinning)
...
ancestor_times = [10, 100, 1000, 5000, 10000, 20000, 40000, 80000, 160000, 320000] #generations in the past to locate ancestors at
...
rule all:
  input:
    expand(anc, CHR=CHRS),
    locations,
    expand(expand(ancestor_locations_blup, zip, CHR=CHRS_rep, locus=sampled_trees_chr, allow_missing=True), M=Ms, T=Ts, s=['All'], t=['All']),
